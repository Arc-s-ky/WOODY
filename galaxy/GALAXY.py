...
special_effects = dict(
    long_paddle=(colors.ORANGE,
                 lambda g: g.hero.bounds.inflate_ip(c.hero_width // 2, 0),
                 lambda g: g.hero.bounds.inflate_ip(-c.hero_width // 2, 0)),
...
class Galaxy(Game):
    def __init__(self):
        Game.__init__(self, 'GALAXY', c.screen_width, c.screen_height, c.background_image, c.frame_rate)
    ...
    def create_hero(self):
        hero = Hero((c.screen_width - c.hero_width) // 2,
                    c.screen_height - c.hero_height * 2,
                    c.hero_width,
                    c.hero_height,
                    c.hero_color,
                    c.hero_speed)
        self.keydown_handlers[pygame.K_LEFT].append(hero.handle)
        self.keydown_handlers[pygame.K_RIGHT].append(hero.handle)
        self.keyup_handlers[pygame.K_LEFT].append(hero.handle)
        self.keyup_handlers[pygame.K_RIGHT].append(hero.handle)
        self.hero = hero
    ...
    def handle_ball_collisions(self):
        ...
        s = self.ball.speed
        edge = intersect(self.hero, self.ball)
        if edge == 'top':
            speed_x = s[0]
            speed_y = -s[1]
            if self.hero.moving_left:
                speed_x -= 1
            elif self.hero.moving_left:
                speed_x += 1
